...
import { fetchProfile } from "../../redux/slices/auth.slice";  // Import the correct action for fetching user data

export default function CheckoutFail() {
  const dispatch = useDispatch();
  
  // Accessing the Razorpay state
    ...
    <Layout>
      <section className="flex flex-col gap-6 items-center py-12 px-3 min-h-[100vh]">
        <div className="flex flex-col dark:bg-gray-800 bg-white gap-12 rounded-lg md:py-10 py-7 md:px-8 md:pt-3 px-3 md:w-[500px] w-full shadow-custom dark:shadow-xl transition duration-300">
          ...
          <Link
            to="/checkout"
            className="bg-red-500 transition-all ease-in-out duration-300 w-full py-3 text-xl font-semibold text-white text-center rounded-bl-lg rounded-br-lg"
          >
            Try Again
          </Link>
        </div>
